...
#[derive(Debug)]
enum ControlMessage {
    SetCaps { pad_name: String, caps: gst::Caps },
    Buffer {
        pad_name: String,
        data: Bytes,
        pts: Option<gst::ClockTime>,
    },
    DropPad { pad_name: String },
    Shutdown,
}
...
impl ElementImpl for MoqSink {
    ...
    fn release_pad(&self, pad: &gst::Pad) {
        if let Some(session) = self.session.lock().unwrap().as_ref() {
            let _ = session
                .sender
                .blocking_send(ControlMessage::DropPad { pad_name: pad.name().to_string() });
        }
...
fn new_decoder(
    ...
    let decoder = moq_mux::import::Decoder::new(
        runtime.broadcast.clone(),
